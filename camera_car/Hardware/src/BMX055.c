...
bool BMX055_init(void)
{
  ...
  /************************************/
    ...
    if(IIC_Write_Reg(IIC_BMX055_ACC_ADR, BMX055_ACC_PMURANGE, 0x05) == false)
        return false;   ///4G      3 2G  5 4G  8 8G  
    Common_delay(10);
    if(IIC_Write_Reg(IIC_BMX055_ACC_ADR, BMX055_ACC_PMUBW, 0x0F) == false)
        return false;     ///1000HZ      
    Common_delay(10);
    if(IIC_Write_Reg(IIC_BMX055_ACC_ADR, BMX055_ACC_PMULPM, 0x00) == false)
  ...
  /************************************/
    ...
    if(IIC_Write_Reg(IIC_BMX055_MAG_ADR, BMX055_MAG_DATARATE, 0x38) == false)
        return false;   ///输出速率30HZ       
    Common_delay(10);
    if(IIC_Write_Reg(IIC_BMX055_MAG_ADR, BMX055_MAG_INTEN, 0x00) == false)
        return false;      ///不使能中断      
    Common_delay(10);
    return true;
}
...
bool BMX055_DataRead(BMX055Datatypedef *Q, uint8 type)
{
  ...
  /************************************/
    if(type)
    {
        if(IIC_Read_Buff(IIC_BMX055_MAG_ADR, BMX055_MAG_XDATALSB, datatemp, 6) == false)
            return false;
