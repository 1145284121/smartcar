...
__ramfunc void IIC_DELAY(uint8 m)/////延时信号
{
  uint8 i = 0;    
  while(m--)
  {
...
__ramfunc void IIC_stop(void)/////停止信号
{ 
    IIC_SDA_H;
    IIC_SCL_L;
    IIC_DELAY(2);
...
__ramfunc void IIC_ack(void)////应答信号
{	
  IIC_SCL_L;
  IIC_DELAY(1);
  IIC_SDA_L;
...
__ramfunc void IIC_noack(void)////无应答信号
{
  IIC_SCL_L;
  IIC_DELAY(1);
  IIC_SDA_H;
